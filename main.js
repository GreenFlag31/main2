...
async function CheckForPossibleID() {
  if (document.querySelector(".regen-contract")) return
  ...
  IDContainer.style.justifyContent = "flex-start"
  IDContainer.style.alignItems = "center"
  const message = document.createElement("p")
  message.classList.add("ID-message")
  message.style.margin = "0 30px 0 0"
  ...
  const supplierContainer = document.querySelector(".supplier-container")
  document.querySelector(".modal-custom-content").insertBefore(IDContainer, supplierContainer)

  if (!isClient) {
    addMessage(message, "Individual Decision is possible")
    return
  }
  CreateLoader("ID-container")
